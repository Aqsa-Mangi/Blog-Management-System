<?php 
	...
	if(isset($_POST['add_post']))
	{
		...
		$inser_query = "INSERT INTO `posts`(added_by_user_role_id,post_title,post_summary,post_description)
		VALUES(?,?,?,?)";

		$stmt = mysqli_prepare($connection,$inser_query);
		$user_role_id = $_SESSION['user']['user_role_id'];

		mysqli_stmt_bind_param($stmt,'isss',$user_role_id,$post_title,$post_summary,$post_desc);
	...
	}if(isset($_POST['update_post']))
	{
		...
		$inser_query = "UPDATE `posts` SET  post_title=?,post_summary=?,post_description=? WHERE post_id = ?";

		$stmt = mysqli_prepare($connection,$inser_query);

		mysqli_stmt_bind_param($stmt,'ssi',$post_title,$post_summary,$post_desc,$post_id);
